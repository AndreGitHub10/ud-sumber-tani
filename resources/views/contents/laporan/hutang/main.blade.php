...
@section('content')
	<div id="main-page">
		...
		<div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
			<div class="breadcrumb-title pe-3">Laporan Hutang</div>
			<div class="ps-3">
		...
		<div class="card">
			...
			<div class="card-body">
				<div class="row mb-4">
					...
					<div class="col-md-3">
						<label for="pembayaran" class="form-label fw-bold">Status</label>
						<select class="single-select validation" id="hutang" onchange="filter()">
...
@push('scripts')
	...
	<script>
		$(async () => {
			...
			datatablePenjualan($('#date_range').val(),$('#hutang').val())
		})
		...
		function initButton(){
			...
			$(".btn-delete").click(async (e) => {
				...
				module.swal.confirm().then(async (e) => {
					if (e.value) {
						const response = await postRequest("{{route('laporan.hutang.destroy')}}", {id: $this.data('id')})
			...
			$(".btn-cancel").click(async (e) => {
				let $this = $(e.currentTarget)
				// return module.swal.warning({text: 'Masih tahap pengembangan!'})
				$this.attr('disabled', true)

				module.swal.confirmEdit().then(async (e) => {
					if (e.value) {
						const response = await postRequest("{{route('laporan.hutang.cancel')}}", {id: $this.data('id')})
						code = response.data.code
		...
		async function datatablePenjualan(date_range=$('#date_range').val(),lunas=$('#hutang').val()){
